...
import { Menu, MenuItem, Button, Fab, Dialog, DialogContent } from "@mui/material";
...
const Main = () => {
  ...
  const handleCameraClose = () => {
    ...
    if (videoRef.current && videoRef.current.srcObject) {
      ...
      console.warn("[WARN] 카메라 스트림이 이미 종료되었거나 정의되지 않았습니다.");
    }
  };
...
const sendImageToBackend = async () => {
  ...
  try {
    ...
    for (let i = 0; i < capturedImages.length; i++) {
      console.log(`[DEBUG] 처리 중인 이미지 ${i + 1}/${capturedImages.length}`);
    ...
    const res = await fetch("https://dd93-203-246-85-142.ngrok-free.app/api/process-images", {
      method: "POST",
      body: formData,
    });
    ...
    // 응답 처리
    if (res.ok) {
      console.log("[SUCCESS] 이미지 업로드 성공!");
      alert("이미지 업로드 성공!");
    } else {
      console.error("[ERROR] 이미지 업로드 실패:", res.status, res.statusText);
      alert(`이미지 업로드 실패: ${res.statusText}`);
    }
  } catch (error) {
    console.error("[ERROR] 이미지 전송 중 오류 발생:", error);
    ...
    <div>
      <div className="list_header">
        ...
        <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
          <MenuItem onClick={() => handleSortChange("최근 등록 순")}>최근 등록 순</MenuItem>
          <MenuItem onClick={() => handleSortChange("소비기한 적게 남은 순")}>소비기한 적게 남은 순</MenuItem>
          <MenuItem onClick={() => handleSortChange("식재료 이름(ㄱ-ㅎ)")}>식재료 이름(ㄱ-ㅎ)</MenuItem>
        </Menu>
      ...
      <div style={{ textAlign: "center", marginTop: "20px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
          {capturedImages.map((image, index) => (
            <img key={index} src={image} alt={`Captured ${index}`} style={{ width: "100px", height: "100px" }} />
          ))}
        </div>
        {capturedImages.length > 0 && (
          <Button variant="contained" color="secondary" onClick={sendImageToBackend} sx={{ mt: 2 }}>
            완료
          </Button>
        )}
      </div>
